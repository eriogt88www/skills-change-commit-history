...
_You removed `***REMOVED***` from the repository's root directory! :tada:_
...
### :keyboard: Activity: Use BFG Repo-Cleaner to remove the `***REMOVED***` file
...
3. Confirm the `***REMOVED***` file is removed from the root directory. The command should return empty.
   ```shell
   find . -name "***REMOVED***"
   ```
4. Search for ***REMOVED*** in the repository's history. The command should return at least 2 commits: the addition of `***REMOVED***` when you copied this template repository, and the removal of `***REMOVED***`.
   ```shell
   git log --stat --all -- ***REMOVED***
   ```
5. Use BFG Repo-Cleaner to delete all references to `***REMOVED***` that exist in the repository.
   ```shell
   bfg --delete-files ***REMOVED***
   ```
...
7. Repeat the search for `***REMOVED***` in the repository's history. This time, the command should return empty.
   ```shell
   git log --stat --all -- ***REMOVED***
   ```
