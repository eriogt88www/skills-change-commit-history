...
We'll start by working with `***REMOVED***` files. These files usually contain sensitive content. For this course, we'll work on removing that file and all traces in the Git history. The first step is to remove the file from repository. We'll alter the history later.
...
### :keyboard: Activity: Remove `***REMOVED***` in the project root directory
...
1. Open your terminal of choice, clone this repository, and switch to your repository directory.
   ...
   ```
2. Delete `***REMOVED***` from the root directory.
   ```shell
   git rm ***REMOVED***
   ```
3. Commit the removal of `***REMOVED***`.
   ```shell
   git commit -m "remove ***REMOVED*** file"
   ```
